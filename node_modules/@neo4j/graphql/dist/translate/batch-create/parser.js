    ...
    const properties = Object.entries(input).reduce((obj, [key, value]) => {
        ...
        if (typeof value === "object" && value !== null && (relationField || !scalarOrEnum)) {
            ...
            if (Array.isArray(value)) {
                obj[key] = new cypher_builder_1.default.List(value.map((GraphQLCreateInput) => inputTreeToCypherMap(GraphQLCreateInput, nodeInput, context, key, relationship)));
                return obj;
            }
...
function parseCreate(input, node, context, counter = 0) {
    const nodeProperties = input.properties;
    raiseOnNotSupportedProperty(node);
    raiseAttributeAmbiguity(input.properties, node);
    const createAST = new GraphQLInputAST_1.CreateAST(counter++, [...nodeProperties], node);
...
function parseNestedCreate(input, node, context, parentNode, relationshipPropertyPath, relationship, counter, edge) {
    ...
    const nestedCreateAST = new GraphQLInputAST_1.NestedCreateAST(counter++, node, parentNode, [...nodeProperties], [...edgeProperties], relationshipPropertyPath, relationship, edge);
    if (input.children.node) {
