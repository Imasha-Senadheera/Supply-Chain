...
exports.CypherOperation = void 0;
const cypher_builder_1 = __importDefault(require("@neo4j/cypher-builder"));
const ReadOperation_1 = require("./ReadOperation");
class CypherOperation extends ReadOperation_1.ReadOperation {
    constructor({ cypherAttributeField, target, relationship, directed, selection, }) {
    ...
    transpile(context) {
        ...
        const fieldSubqueries = cypher_builder_1.default.concat(...this.getFieldsSubqueries(nestedContext), ...this.getCypherFieldsSubqueries(nestedContext));
        ...
        const authClauses = authPredicates.length
            ? [...authSubqueries, new cypher_builder_1.default.With("*").where(cypher_builder_1.default.and(...authPredicates))]
            : [];
        const ret = this.getReturnClause(nestedContext, context.returnVariable);
        const clause = cypher_builder_1.default.concat(matchClause, fieldSubqueries, ...authClauses, ret);
        return {
            clauses: [clause],
    ...
    getReturnClause(context, returnVariable) {
        ...
        if (context.shouldCollect) {
            returnExpr = cypher_builder_1.default.collect(context.target);
        }
        else {
            returnExpr = context.target;
        }
        if (context.shouldCollect && !this.cypherAttributeField.typeHelper.isList()) {
            returnExpr = cypher_builder_1.default.head(returnExpr);
        }
...
exports.CypherOperation = CypherOperation;
//# sourceMappingURL=CypherOperation.js.map